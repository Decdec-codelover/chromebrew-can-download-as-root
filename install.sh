...
case "${this_installer}" in
  63)
    ...
    echo_info "Current Decbrew Installer is from $(curl -s "https://api.github.com/repos/Decdec-codelover/Decbrew-can-download-as-root/commits?path=install.sh&page=1&per_page=1" | jq -r '.[0].commit.committer.date')"
    ;;
  install.sh)
...
set_trap() { trap "echo_error 'An error occured during the installation using Decbrew installer ${CREW_INSTALLER_VERSION} :/'" ERR; }
set_trap
...
if [ -f /etc/lsb-release ]; then
  if [[ ! "$(lsbval CHROMEOS_RELEASE_TRACK)" =~ [ltc|stable]-channel && "${CREW_FORCE_INSTALL}" != '1' ]]; then
    echo_error "The beta, dev, and canary channel are unsupported by Decbrew."
    echo_info "Run 'CREW_FORCE_INSTALL=1 bash <(curl -Ls git.io/vddgY) && . ~/.bashrc' to perform install anyway."
    exit 1
  fi
...
set -a
# Default Decbrew repo values.
: "${OWNER:=Decdec-codelover}"
: "${REPO:=Decbrew-can-download-as-root}"
: "${BRANCH:=master}"
: "${CREW_BRANCH:=${BRANCH}}"
...
if grep -s "fma4" /proc/cpuinfo ; then
  ...
  echo_info "If this happens, please report them to: https://github.com/Decdec-codelover/Decbrew-can-download-as-root/issues"
  ...
  if [ "${CREW_AMD_INSTALL}" == "1" ]; then
    ...
    # See https://github.com/Decdec-codelover/Decbrew-can-download-as-root/issues/8823
    echo 0 | sudo tee /proc/sys/kernel/randomize_va_space
  fi
fi

echo_success "Welcome to Decbrew!"
...
# Download the Decbrew repository.
...
function get_pkg_version () {
  grep "\ \ version" "${1}" | head -n 1 | sed "s/#{LIBC_VERSION}/$LIBC_VERSION/g" | sed "s/#{@gcc_libc_version}/$LIBC_VERSION/g" | sed "s/#{CREW_PY_VER}/py$CREW_PY_VER/g"| sed "s/#{CREW_RUBY_VER}/$CRE[...]
}
...
function update_and_install_rubygems () {
  ...
  ${PREFIX_CMD} gem outdated | cut -d " " -f 1 | grep -v repl_type_completor | xargs -I % bash -c 'export pkg=% ; grep -q no_compile_needed /usr/local/lib/crew/packages/ruby_${pkg//-/_}.rb && (echo "U[...]
...
# containers.
if [[ "${ARCH}" == "x86_64" ]]; then
  export LD_LIBRARY_PATH="$CREW_PREFIX/lib64:/usr/lib64:/lib64"
else
  export LD_LIBRARY_PATH="${CREW_PREFIX}/lib:/usr/lib:/lib"
fi
...
# Work around https://github.com/Decdec-codelover/Decbrew-can-download-as-root/issues/3305.
...
echo_info "Installing core Decbrew packages...\n"
${PREFIX_CMD} crew install core -f

if [ -n "${BUILDESSENTIALS}" ]; then
...
if ! "${CREW_PREFIX}"/bin/git version &> /dev/null; then
  ...
  echo_error "https://github.com/Decdec-codelover/Decbrew-can-download-as-root/issues\n\n"
else
...
Decbrew provides nano, vim and emacs as default TUI editor options."

echo_success "Decbrew installed successfully and package lists updated."
